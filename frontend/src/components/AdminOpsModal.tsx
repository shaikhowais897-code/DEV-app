  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-6 overflow-y-auto font-sans">
      <div className="bg-[#151718] border border-white/15 rounded-3xl max-w-6xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[94vh] animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          {onResetDefaultCatalog && (
                            <button
                              ...
                              className="px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/15 text-white/70 hover:text-white text-xs font-semibold rounded-xl flex items-center gap-1.5 cursor-pointer transition-colors"
                              title="Reset and restore original catalog"
                            >
        ...
        {/* Modal: Reset Catalog to Defaults Confirmation */}
        {showResetCatalogConfirm && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
            <div className="bg-[#181a1b] border-2 border-[#c0342c]/50 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl animate-in zoom-in-95">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-[#c0342c]/20 border border-[#c0342c]/40 flex items-center justify-center text-[#ffb4aa] shrink-0">
                  <RotateCcw className="w-6 h-6" />
                </div>
                <div>
                  <h4 className="text-base font-bold text-white font-display">Restore Default Master Catalog?</h4>
                  <p className="text-xs text-white/50">Reset all titles back to original Whoosh presets.</p>
                </div>
              </div>

              <div className="p-3 bg-[#121414] border border-white/10 rounded-xl text-xs text-white/70 leading-relaxed">
                This will restore the original built-in movie and series catalog. Any titles you added or deleted locally will be reset.
              </div>

              <div className="flex items-center justify-end gap-2 pt-2 border-t border-white/10">
                <button
                  type="button"
                  onClick={() => setShowResetCatalogConfirm(false)}
                  className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-xs font-semibold rounded-xl cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  type="button"
                  onClick={() => {
                    if (onResetDefaultCatalog) {
                      onResetDefaultCatalog();
                    }
                    setShowResetCatalogConfirm(false);
                  }}
                  className="px-5 py-2 bg-[#c0342c] hover:bg-[#d63d34] text-white text-xs font-bold rounded-xl shadow-lg shadow-[#c0342c]/40 flex items-center gap-1.5 cursor-pointer active:scale-95 transition-all"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
